...
class Unity_Env(object):
    ...
    def __init__(self,
        ...
        for agent_index, action_type in enumerate(self.action_type):
            if action_type:
                ...
                self.action_space.append(
                    self.env.behavior_specs.get(
                        list(self.agent_names)[agent_index]).action_spec.discrete_branches)
    ...
    def step(self, actions):
        ...
        for behavior_name_index, behavior_name in enumerate(self.agent_names):
            ...
            if self.action_type[behavior_name_index]:
                ...
                # index = 0
                # agent_action = []
                # for i in self.action_space[behavior_name_index]:
                #     agent_action.append(actions[behavior_name_index][index:index+i].argmax(-1))
                #     index = index + i
                action.add_discrete(np.asarray([[actions[behavior_name_index]]]))#.cpu()
                action.add_continuous(np.asarray([[]]))
    ...
    def get_info(self):
        ...
        env_info["obs_shape"] = self.observation_space[0]
        env_info["n_actions"] = self.action_space[0]
        for i in self.observation_space[0]:
            if len(i) == 3:
                a = (i[0],i[1],i[2]*self.n)
                env_info["state_shape"].append(a)
            else:
                env_info["state_shape"].append((i[0] * self.n,))
        return env_info
