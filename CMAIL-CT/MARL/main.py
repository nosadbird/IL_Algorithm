...
def train(args, return_early=False):
    ...
    for j in range(args.num_updates):
        ...
        if j%args.log_interval == 0:
            ...
            print("Updates {}: Num timesteps {}: Mean reward {:.4f}: Entropy {:.4f}: Value loss {:.4f}: Policy loss {:.4f}:".
                  format(j, total_num_steps, sum(mean_reward), dist_entropy[0], value_loss[0], action_loss[0]))
            if not args.test:
                for idx in range(n):
