...
# wandb.init(project="605car", entity="nxbb")
...
wandb.init(project="研究点1", entity="nxbb")
...
parser.add_argument('--env', type=str, default="Walker2d-v2", metavar='G',
...
discriminator = Discriminator(num_inputs*2, args.hidden_dim).to(device)
...
agent = SAC(env, 0.99, 0.005, 100000, 1e-4, 1e-4, 1e-4)
...
def expert_reward(state, nstate):
    # states = np.concatenate(states)
    # ns = np.concatenate(nstates)
    state = np.expand_dims(state, axis=0)
    nstate = np.expand_dims(nstate, axis=0)
    state_ns = torch.Tensor(np.concatenate([state, nstate], 1)).to(device)
    return -F.logsigmoid(discriminator(state_ns)).cpu().detach().numpy()[0][0]


#plabel = ''
try:
    # file = open('./demonstrations/ball32-5000.pkl', 'rb')
    # expert_traj = pickle.load(file)
    expert_traj = np.load("./demonstrations/Walker2dV2.npy")
...
for i_episode in range(2000):#500 args.num_epochs
    memory = Memory()
    ...
    while num_steps < 5000:#5000 不管几个episode，够5000就向下
        state = env.reset()
        #state = state.astype('double')


        reward_sum = 0
        for t in range(1500): # 一个episode最多10000步
            ...
            reward = expert_reward(state, next_state)
            agent.buffer.push((state,action,reward,next_state,mask))
    ...
    #     agent.buffer.push((states[idx][0], actions[idx], rewards[idx][0], mem_next[idx], mem_mask[idx]))
    ...
    states = torch.from_numpy(np.float32(np.concatenate(states)))
    n_states = torch.from_numpy(np.float32(np.concatenate(next_states)))
    
    idx = np.random.randint(0, len(expert_traj)-1, num_steps)
    
    expert_state_ns = expert_traj[idx, :]
    #expert_state_ns = random.sample(expert_traj, num_steps)
    ...
    if i_episode % args.log_interval == 0:
        wandb.log({"Walker2d-v2": np.mean(reward_batch)})
        print('Episode {}\tAverage reward: {:.2f}\tnum_step: {:.2f}\tLoss (disc): {:.2f}'.format(i_episode, np.mean(reward_batch), num_steps, disc_loss.item()))
